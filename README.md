...
- Models are typically billed per token. Total cost per request depends on the number of input tokens (your prompt + retrieved context) plus the output tokens generated by the model. Check Groq's pricing page for exact rates.
- Keep input size small: limit `n_results`, shorten or summarize retrieved chunks, and keep system/user messages concise.
- Use generation caps (for example, `max_tokens`) if supported by the SDK to bound output length and cost.
- Reduce calls and reuse work: cache common answers, precompute embeddings, and batch related queries where possible.
- Monitor and enforce limits: log token counts per request (input/output), set request quotas, and add rate limiting to the API to avoid runaway costs.
- Tip: summarizing long documents before storing them or storing both full and short summaries lets you choose cheaper, shorter context for typical queries.
